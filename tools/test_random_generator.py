...
def test_get_constraints():
    ...
    assert get_constraints(test_schema_dict["topLevel"]["number"]) == {
        "min": 0,
        "max": 10,
    }
...
def test_from_dict():
    result = [
        {"listOfNum": val.List(val.Number(min=0))},
        {"optionalString": val.String()},
    ]
...
def test_from_list():
    ...
    assert (
        from_list("listOfIncludes", test_schema_dict["topLevel"]["listOfIncludes"])
        == result
    )
...
def test_from_include():
    result = [
        {"listOfNum": val.List(val.Number(min=0))},
        {"optionalString": val.String(required=False)},
    ]
    assert (
        from_include(
            test_schema_dict["topLevel"]["singleInclude"], includes=test_schema_includes
        )
        == result
    )
