...
class ModelArray(ModelBase):
    ...
    def to_json(self, **extras):
        ...
        if isinstance(self.item, ModelArray):
            items.update(self.item.to_explicit_json())
        else:
            items.update(self.item.to_json())
        # remove required from item
        items.pop("required", False)
        items.pop("label.en", None)
...
class ModelChoose(ModelBase):
    ...
    def to_json(self):
        ret = super().to_json()
        ret["oneof"] = [{"discriminator": k, "type": v.to_json()["use"]} for k, v in self.subschemas.items()]
...
class ModelLink(ModelBase):
    ...
    def to_json(self):
        ...
        link_props = {
            ...
            "target": self.model or self.target,
            "keys": self.fields,
        }
...
class ModelVocabulary(ModelLink):
    ...
    def to_json(self):
        ...
        if VOCABULARY_CUSTOM_FIELD_KEYS[self.vocabulary] is None:
            raise Exception(f"Vocabulary {self.vocabulary} is not supported, please add it to VOCABULARY_CUSTOM_FIELD_KEYS in yamale2oarepo_config.py. SELF_FIELDS: {self.fields}")
...
def parse(d, path, includes, default_search=False, label=None):
    ...
    # TODO Add data type url for Url class instead of keyword
    elif clz is Url:
        ...
        return ModelPrimitive(d, "int", path, default_search, label)
    elif clz is Keyword:
        return parse_keyword(d, path, default_search, label)
...
def load_excluded_types(yaml_file):
    import yaml
    with open(yaml_file, "r") as f:
        excluded_types_dict = yaml.safe_load(f)
    return set(excluded_types_dict.keys())
...
def run(input_file, debug, out_dir, only_defs, include, exclude_types):
    if debug:
    ...
    def convert_use_to_type(json_dict):
        if isinstance(json_dict, dict):
            if "use" in json_dict:
                json_dict["type"] = json_dict.pop("use")
            for v in json_dict.values():
                convert_use_to_type(v)
    ...
    for json_dict, name, model_package in out:
        print(f"Generating {name} for {model_package}")
